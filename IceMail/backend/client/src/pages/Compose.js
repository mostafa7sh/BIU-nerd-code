        ...
        if (res.ok) {
          ...
          setAttachments((data.attachments || []).map(att => {
            if (typeof att === 'string') {
              return { name: att.split('/').pop(), data: att };
            }
            const base64 = att.data ? (typeof att.data === 'string' ? att.data : btoa(String.fromCharCode(...new Uint8Array(att.data.data || att.data)))) : '';
