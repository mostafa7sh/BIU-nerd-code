...
export default function Inbox() {
  ...
  const handleSelectAll = () => {
    if (selectedMailIds.length === mails.length) {
      ...
      const allIds = mails.map((m) => m._id);
      ...
      <div className="inbox-container">
        ...
        {selectedMailIds.length > 0 && (
          <div className="read-actions">
            <button className="icloud-btn" onClick={() => markSelectedAsReadOrUnread(true)}>
              Mark as Read
            </button>
            <button className="icloud-btn secondary" onClick={() => markSelectedAsReadOrUnread(false)}>
              Mark as Unread
            </button>
          </div>
        )}
        ...
        {mails.length === 0 ? (
          <p className="empty-message">No emails found.</p>
        ) : (
          mails.map((mail) => (
            <MailCard
              key={mail._id}
