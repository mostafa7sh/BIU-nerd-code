...
export default function MailCard({ mail, selectable, isSelected, toggleSelect, markAsReadToggle }) {
  ...
  const [isFlagged, setIsFlagged] = useState(mail.isImportant || false);
  ...
  const handleFlagClick = async (e) => {
    e.stopPropagation();
    try {
      const res = await authFetch(`/api/mails/${mail._id}/important`, {
        method: 'POST'
      });
  ...
  return (
    <div className={`mail-card ${mail.isRead ? 'read' : 'unread'}`} onClick={handleClick}>
      ...
      <div className="tooltip">
        <img
          src={mail.fromProfilePic || DefaultAvatar}
      ...
      <div className="read-toggle" onClick={(e) => {
        e.stopPropagation();
        markAsReadToggle(mail._id, !mail.isRead);
      }}>
      </div>
    </div>
  );
